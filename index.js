...
function init() {
	...
	postcss(oldie(atom.config.get('oldie'))).process(text, {
		parser: postcssSafeParser
	}).then(result => {
		result.warnings().forEach(x => {
			console.warn(x.toString());
			atom.notifications.addWarning('oldie', {detail: x.toString()});
		});
		...
		console.error(err);
		atom.notifications.addError('oldie', {detail: err.message});
	});
}
...
export const activate = () => {
	atom.commands.add('atom-workspace', 'oldie', init);
};
